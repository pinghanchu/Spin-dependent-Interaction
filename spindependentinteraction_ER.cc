...
const long double amp = 0.005*unit; // source mass amplitude; modulation amplitude (5mm)
const long double gap = 0.005*unit; // gap between source mass and detector (5mm) 
...
const long double bounds2[6]={maxlength*unit,gap+amp,1.*pi,0.,2.*pi,0.};
...
//const long double D[3] = {cellsize*0.5*unit,0.0*unit, 0.0*unit};
// the distance between the mass and the detector due to the off-diagonal setup
